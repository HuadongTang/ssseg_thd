...
class SemanticLevelContext(nn.Module):
    def __init__(self, feats_channels, transform_channels, concat_input=False, norm_cfg=None, act_cfg=None):
        ...
        if concat_input:
            self.bottleneck = nn.Sequential(
                nn.Conv2d(feats_channels * 3, feats_channels, kernel_size=3, stride=1, padding=1, bias=False),
        ...
        BuildNormalization(512, norm_cfg=norm_cfg),
        BuildActivation(act_cfg),
    ...
    def forward(self, x, preds, feats_il):
        ...
        value = inputs
        proj_query = self.query_conv(value).view(batch_size, -1, w * h).permute(0, 2, 1)
        proj_key = self.key_conv(value).view(batch_size, -1, w * h)
        ...
        intra_context = intra_context.div(np.prod([h, w]))
        ...
        inter_context = inter_context.div(np.prod([h, w]))
        ...
        intra_feats = self.intra_feats_conv(torch.cat([inputs, intra_context], dim=1))
        inter_feats = self.inter_feats_conv(torch.cat([inputs, inter_context], dim=1))
        ...
        intra = self.correlate_net(intra_feats, feats_sl)
        inter = self.correlate_net(inter_feats, feats_sl)
        # feats_sl = self.correlate_net(inputs, feats_sl)
        if hasattr(self, 'bottleneck'):
            feats_sl = self.bottleneck(torch.cat([feats_il, intra, inter], dim=1))
        return feats_sl, context_prior_map