...
class MemoryNet(BaseSegmentor):
    ...
    def forward(self, x, targets=None, **kwargs):
        ...
        # feed to memory
        # memory_in = self.bottleneck(backbone_outputs[-1])
        ...
        if self.mode == 'TRAIN':
            ...
            apd_pred = self.get_adaptive_perspective(x=memory_input, y=targets['seg_target'].unsqueeze(1), new_proto=stored_memory.detach().squeeze(),
                                                 proto=stored_memory.squeeze())
            # N, C, W ,H = memory_output.size()
            # softmax_pred_T = F.softmax(apd_pred.view(-1, W * H), dim=1)
            # logsoftmax = torch.nn.LogSoftmax(dim=1)
            # loss = torch.sum(- softmax_pred_T * logsoftmax(memory_output.view(-1, W * H) / self.tau)) * (self.tau ** 2)
            # kl_loss = self.loss_weight * loss / (C * N)
            # N, C, W, H = memory_output.size()
            # y_s = memory_output.transpose(1, 3).reshape(-1, C)
            # y_t = apd_pred.transpose(1, 3).reshape(-1, C)
            # y_s = y_s.softmax(dim=1)
            # y_t = y_t.softmax(dim=1)
            # inter_loss = inter_class_relation(y_s, y_t)
            # intra_loss = intra_class_relation(y_s, y_t)
            # kl_loss = intra_loss + inter_loss
            ...
            losses_log_dict['kl_loss'] = kl_loss.item()
